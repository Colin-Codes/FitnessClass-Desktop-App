...
*/
    ...
    public String reportMonthlyExerciseChampion(int intMonth, int intYear)
    {
        ...
        {
            BigDecimal highestExerciseTakings = new BigDecimal(0.0).setScale(2, RoundingMode.HALF_UP);
            String ReportLine = new String("");
            Integer championExerciseID = new Integer(0);
            Exercise championExercise;
            for (Exercise currentExercise : clubExerciseRegister.values())
            {
                BigDecimal currentExerciseTakings = new BigDecimal(0.0).setScale(2, RoundingMode.HALF_UP);
                for (Class currentClass : clubClassRegister.values())
                {
                    ...
                    if (currentYear == intYear && currentMonth == intMonth && currentClass.getClassExercise().equals(currentExercise))
                    {
                        currentExerciseTakings = currentExerciseTakings.add(currentClass.getTakings());
                    }
                }
                if (currentExerciseTakings.compareTo(highestExerciseTakings) > 0) 
                {
                    championExerciseID = currentExercise.getID();
                    highestExerciseTakings = currentExerciseTakings;
                }
            }
            if (highestExerciseTakings.compareTo(new BigDecimal(0.0)) > 0)
            {
                championExercise = clubExerciseRegister.get(championExerciseID);
                ReportLine = championExercise.getName() + ", Takings: " + highestExerciseTakings.toString();
            }       
            else
            {
                ReportLine = "No exercises have yet taken any money";
            }
            return ReportLine;
        }
    }
    
    public ArrayList<String> reportMonthlyExerciseResults(int intMonth, int intYear)
    {        
        ...
        else
        {
            ...
            for (Exercise currentExercise : clubExerciseRegister.values())
            {
                    ...
                    if (currentYear == intYear && currentMonth == intMonth && currentClass.getClassExercise().equals(currentExercise))
                    {
                        ...
                        for (Booking currentBooking : clubBookingRegister.values())
                        {
                            ...
                            if (BookingClass.equals(currentClass))
                            {
                                ...
                                {
                                    case None:
                                        //Lack of a rating should not affect the overall result - therefore the mean should be unaffected.
                                        notRated++;
                                        break;
                                    case One:
                                        count++;
                ...
                double dblBookings = count + notRated;
                String ReportLine = currentExercise.getName() + ", Bookings: " + Math.round(dblBookings) + averageRating;
